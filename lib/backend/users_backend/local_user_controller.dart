...
class LocalUserController {
  ...
  LocalUserController(
      {required this.userProvider, LocalUserRepository? repository}) {
    userRepository = repository ?? LocalUserRepository();
  }

  Future<void> getAllUsersFromFirebase(
      {bool isRefresh = true, bool isNotify = true}) async {
    try {
      ...
      if (isRefresh) {
        ...
        userProvider.setHasMoreUsers =
            true; // flag for more products available or not
        userProvider.setLastDocument =
      ...
      MyPrint.printOnConsole(
          "Documents Length in Firestore for Admin Users:${querySnapshot.docs.length}");

      if (querySnapshot.docs.length <
          MyAppConstants.userDocumentLimitForPagination)
        userProvider.setHasMoreUsers = false;
      ...
      MyPrint.printOnConsole(
          "Final User Length in Provider:${userProvider.usersListLength}");
    } catch (e, s) {
      MyPrint.printOnConsole(
          "Error in get User List form Firebase in user Controller $e");
      MyPrint.printOnConsole(s);
    }
  }

  Future<void> getAllDisabledUsersFromFirebase(
      {bool isRefresh = true, bool isNotify = true}) async {
    try {
      MyPrint.printOnConsole("In Side The Method");
      ...
      if (isRefresh) {
        MyPrint.printOnConsole("Refresh");
        userProvider.setHasMoreDisabledUsers =
            true; // flag for more products available or not
        userProvider.setDisabledUsersLastDocument =
            null; // flag for last document from where next 10 records to be fetched
      ...
      Query<Map<String, dynamic>> query = FirebaseNodes.usersCollectionReference
          .limit(MyAppConstants.userDocumentLimitForPagination)
          .orderBy("createdTime", descending: true)
          .where('adminEnabled', isEqualTo: false);

      //For Last Document
      DocumentSnapshot<Map<String, dynamic>>? snapshot =
          userProvider.getDisabledUsersLastDocument;
      if (snapshot != null) {
        MyPrint.printOnConsole("LastDocument not null");
        query = query.startAfterDocument(snapshot);
      } else {
      ...
      if (querySnapshot.docs.length <
          MyAppConstants.userDocumentLimitForPagination)
        userProvider.setHasMoreDisabledUsers = false;
      ...
      MyPrint.printOnConsole("Final User Length From Firestore:${list.length}");
      MyPrint.printOnConsole(
          "Final User Length in Provider:${userProvider.disabledUsersListLength}");
    } catch (e, s) {
      MyPrint.printOnConsole(
          "Error in get Disabled User List form Firebase in user Controller $e");
      MyPrint.printOnConsole(s);
    }
  }

  Future<void> EnableDisableUserInFirebase(
    ...
    try {
      await FirebaseNodes.userDocumentReference(userId: id)
        ...
        userProvider.updateEnableDisableOfList(
            editableData["adminEnabled"], listIndex);
      });
    } catch (e, s) {
      MyPrint.printOnConsole(
  ...
  Future<void> EnableDisableDisabledUserInFirebase(
      {required Map<String, dynamic> editableData,
      required String id,
      required int listIndex}) async {
    try {
      await FirebaseNodes.userDocumentReference(userId: id)
          ...
          .then((value) {
        MyPrint.printOnConsole("user data: ${editableData["adminEnabled"]}");
        userProvider.updateEnableDisableDisabledOfList(
            editableData["adminEnabled"], listIndex);
      });
    } catch (e, s) {
      MyPrint.printOnConsole(
